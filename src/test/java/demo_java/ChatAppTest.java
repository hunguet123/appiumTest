...
public class ChatAppTest extends BaseScreen {
    // Tạo một Map lưu trữ tài khoản
    List<Map<String, String>> accountTest;
    ...
    public void test_login() throws InterruptedException, IOException {
        ...
        String fileName = "resultLogin/login_test_" + timestamp.getTime() / 1000L + ".txt";
        ...
        for (Map<String, String> entry : accountTest) {
            MobileElement btnLogin = driver.findElementById("com.example.chatappfirebase:id/btnLogin");
            btnLogin.click();
            Thread.sleep(2000);

            MobileElement emailInput = (MobileElement) driver.findElement(By.id("com.example.chatappfirebase:id/ed_email"));

            MobileElement passwordInput = (MobileElement) driver.findElementById("com.example.chatappfirebase:id/ed_password");
            //email fill
            emailInput.click();
            emailInput.sendKeys(entry != null ? entry.get("email") : "");

            //password fill
            passwordInput.click();
            passwordInput.sendKeys(entry != null ? entry.get("password") : "");
            ...
            //press Login button
            MobileElement loginButton = driver.findElementById("com.example.chatappfirebase:id/btnLogin");
            loginButton.click();
            Thread.sleep(2000);
            ...
            try {
                driver.findElementById("com.example.chatappfirebase:id/text_input_end_icon");
                try {
                    myWriter.write("Account with email " + entry.get("email") + " and password " + entry.get("password") + " is error account" + "\n");
                    extentTest.log(Status.INFO, "Account with email " + entry.get("email") + " and password " + entry.get("password") + " is error account");
                } catch (IOException e) {
                ...
                try {
                    myWriter.write("Account with email " + entry.get("email") + " and password " + entry.get("password") + " is true account" + "\n");
                    extentTest.log(Status.INFO, "Account with email " + entry.get("email") + " and password " + entry.get("password") + " is true account");
                    Thread.sleep(1000);
